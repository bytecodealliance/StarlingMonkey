...
#define WIZER_INIT(init_func)                                                  \
  ...
  __attribute__((export_name("wizer-initialize"))) void __wizer_initialize() { \
    ...
    /* of the global constructors. For reactor modules, _initialize is   */     \
    /* called first by Wizer::run()/wasmtime-wizer which already runs    */     \
    /* __wasm_call_ctors. Calling it again traps in wasi-sdk v30+.       */     \
    /* __wasm_call_ctors(); */                                                 \
  ...
  __attribute__((export_name("wizer.resume"))) void __wizer_resume() {         \
    ...
    /* destructors as well.                                             */     \
    __wasm_call_dtors();                                                       \
    /* If main returned non-zero code, exit with that code. */                  \
    if (r != 0) {                                                              \
      exit(r);                                                                 \
    }                                                                          \
  }
