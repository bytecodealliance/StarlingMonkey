...
bool validate_event_target_args(JSContext *cx, HandleValue type, HandleValue callback,
                                const char *tag) {
  if (!type.isString()) {
    return api::throw_error(cx, api::Errors::TypeError, tag, "type", "be a string");
  }

  if (!callback.isObject()) {
    return api::throw_error(cx, api::Errors::TypeError, tag, "callback", "be an object");
  }

  JS::RootedObject callback_obj(cx, &callback.toObject());
  if (!JS::IsCallable(callback_obj)) {
    RootedValue callback_val(cx);
    if (!JS_GetProperty(cx, callback_obj, "handleEvent", &callback_val)) {
      return false;
    }

    if (callback_val.isNullOrUndefined()) {
      return api::throw_error(cx, api::Errors::TypeError, tag, "callback", "is EventListener interface");
    }
  }
...
bool EventTarget::addEventListener(JSContext *cx, unsigned argc, JS::Value *vp) {
  ...
  RootedValue type_val(cx, args.get(0));
  RootedValue callback_val(cx, args.get(1));
  RootedValue opts_val(cx, args.get(3));

  if (!validate_event_target_args(cx, type_val, callback_val, "EventTarget.addEventListener")) {
    return false;
  }

  // Already checked by validate function.
  MOZ_ASSERT(callback_val.isObject());
...
bool EventTarget::removeEventListener(JSContext *cx, unsigned argc, JS::Value *vp) {
  ...
  if (!validate_event_target_args(cx, type_val, callback_val, "EventTarget.removeEventListener")) {
    return false;
  }
...
bool EventTarget::dispatchEvent(JSContext *cx, unsigned argc, JS::Value *vp) {
  ...
  RootedValue event_val(cx, args.get(0));
  if (!Event::is_instance(event_val)) {
    return api::throw_error(cx, api::Errors::TypeError, "EventTarget.dispatch", "event", "be an Event");
  }
  ...
  if (!dispatch(cx, self, event, nullptr, args.rval())) {
    return false;
  }
...
//
// StarlingMonkey currently doesn't support Node objects (i.e. every check with `isNode()` returns false),
// which means we don't need to build a full event propagation path that walks parent nodes, deals with
// shadow DOM retargeting, or handles activation behaviors. In a simplified version we assume that the
...
bool EventTarget::dispatch(JSContext *cx, HandleObject self, HandleObject event, HandleObject target_override, MutableHandleValue rval) {
  ...
  Event::set_target(event, nullptr);
...
bool EventTarget::invoke_listeners(JSContext *cx, HandleObject target, HandleObject event) {
  ...
  JS::RootedVector<EventListener> list_clone(cx);
  ...
  *found = false;
  ...
  for (auto listener : list) {
    ...
    *found = true;
    ...
    RootedObject callback_obj(cx, &listener.callback.toObject());
    ...
    if (JS::IsCallable(callback_obj)) {
      RootedValue thisv(cx, JS::NullValue());
      if (!JS::Call(cx, thisv, callback_val, args, &rval)) {
        return false;
      }
    } else {
