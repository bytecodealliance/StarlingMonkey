...
use crate::raw::{JSContext, JSErrorFormatString, JSExnType, JS_ReportErrorNumberUTF8};
use libc;
use std::ffi::CString;
use std::{mem, os, ptr};
use std::ptr::addr_of;
...
/// Helper fn to throw a javascript error with the given message and exception type.
/// Reuse the jsapi error codes to distinguish the exception_type
/// passed back to the get_error_message callback.
unsafe fn throw_js_error(cx: *mut JSContext, error: &str, exception_type: JSExnType) {
...
pub unsafe fn throw_type_error(cx: *mut JSContext, error: &str) {
    throw_js_error(cx, error, JSExnType::JSEXN_TYPEERR);
}
...
pub unsafe fn throw_internal_error(cx: *mut JSContext, error: &str) {
    throw_js_error(cx, error, JSExnType::JSEXN_INTERNALERR);
}
