...
struct TaskQueue {
  std::vector<core::AsyncTask*> tasks = {};
...
void EventLoop::queue_async_task(AsyncTask* task) {
  queue.get().tasks.emplace_back(task);
}

bool EventLoop::cancel_async_task(Engine* engine, const int32_t id) {
...
bool EventLoop::run_event_loop(Engine* engine, double total_compute, MutableHandleValue result) {
  ...
  do {
    ...
    if (has_pending_async_tasks()) {
      ...
      const auto index = AsyncTask::select(tasks);
