...
inline JS::Result<mozilla::Ok> base64Decode4to3(std::string_view input,
                                                std::string &output,
                                                const uint8_t *decodeTable) {
  uint8_t w, x, y, z;
...
inline JS::Result<mozilla::Ok> base64Decode3to2(std::string_view input,
                                                std::string &output,
                                                const uint8_t *decodeTable) {
  uint8_t w, x, y;
  // 8.1 Find the code point pointed to by position in the second column of
...
inline JS::Result<mozilla::Ok> base64Decode2to1(std::string_view input,
                                                std::string &output,
                                                const uint8_t *decodeTable) {
  uint8_t w, x;
  // 8.1 Find the code point pointed to by position in the second column of
...
JS::Result<std::string>
forgivingBase64Decode(std::string_view data,
                      const uint8_t *decodeTable = base64DecodeTable) {
  // 1. Remove all ASCII whitespace from data.
  // ASCII whitespace is U+0009 TAB, U+000A LF, U+000C FF, U+000D CR, or U+0020
  // SPACE.
  auto hasWhitespace =
      std::find_if(data.begin(), data.end(), &isAsciiWhitespace);
  std::string dataWithoutAsciiWhitespace;

  if (hasWhitespace) {
    dataWithoutAsciiWhitespace = data;
    dataWithoutAsciiWhitespace.erase(
        std::remove_if(dataWithoutAsciiWhitespace.begin() +
                           std::distance(data.begin(), hasWhitespace),
                       dataWithoutAsciiWhitespace.end(), &isAsciiWhitespace),
...
bool atob(JSContext *cx, unsigned argc, Value *vp) {
  ...
  RootedString decodedData(
      cx, JS_NewStringCopyN(cx, decoded.c_str(), decoded.length()));
  if (!decodedData) {
    return false;
  }
...
inline void base64Encode2to4(std::string_view data, std::string &output,
  ...
  output +=
      encodeTable[(uint32_t)(((src0 & 0x03) << 4) | ((src1 >> 4) & 0x0F))];
