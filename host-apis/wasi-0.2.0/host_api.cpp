...
typedef wasi_http_0_2_0_types_borrow_future_incoming_response_t
...
template <> struct HandleOps<OutputStream> {
  ...
  static constexpr const auto borrow_owned =
      wasi_io_0_2_0_streams_borrow_output_stream;
};
...
uint64_t MonotonicClock::resolution() {
  return wasi_clocks_0_2_0_monotonic_clock_resolution();
}
...
HttpHeaders::HttpHeaders() {
  this->handle_state_ =
      new HandleState(wasi_http_0_2_0_types_constructor_fields().__handle);
}
...
string_view HttpRequestResponseBase::url() {
  ...
  success =
      wasi_http_0_2_0_types_method_incoming_request_authority(borrow, &authority);
  MOZ_RELEASE_ASSERT(success);

  bindings_string_t path;
  success =
      wasi_http_0_2_0_types_method_incoming_request_path_with_query(borrow, &path);
  MOZ_RELEASE_ASSERT(success);
...
Result<uint64_t> HttpOutgoingBody::capacity() {
  ...
  uint64_t capacity = 0;
  wasi_io_0_2_0_streams_stream_error_t err;
  if (!wasi_io_0_2_0_streams_method_output_stream_check_write(borrow, &capacity,
                                                                            &err)) {
    return Result<uint64_t>::err(154);
  }
...
Result<Void> HttpOutgoingBody::close() {
...

  {
    wasi_io_0_2_0_streams_stream_error_t err;
    bool success =
        wasi_io_0_2_0_streams_method_output_stream_blocking_flush(borrow, &err);
    MOZ_RELEASE_ASSERT(success);
    // TODO: handle `err`
  }
...
Result<PollableHandle> HttpOutgoingBody::subscribe() {
  ...
  if (state->pollable_handle_ == INVALID_POLLABLE_HANDLE) {
    ...
    state->pollable_handle_ =
        wasi_io_0_2_0_streams_method_output_stream_subscribe(borrow).__handle;
  }
...
wasi_http_0_2_0_types_method_t http_method_to_host(string_view method_str) {
  ...
  if (method_str.empty()) {
    return wasi_http_0_2_0_types_method_t{
  ...
  auto val = bindings_string_t{reinterpret_cast<uint8_t *>(const_cast<char *>(method)),
                               method_str.length()};
  return wasi_http_0_2_0_types_method_t{
      WASI_HTTP_0_2_0_TYPES_METHOD_OTHER, {val}};
}
...
HttpOutgoingRequest *HttpOutgoingRequest::make(string_view method_str, optional<HostString> url_str,
  ...
  auto handle = wasi_http_0_2_0_types_constructor_outgoing_request(
      {headers->handle_state_->handle});
  {
    ...
    // TODO: error handling on result
    wasi_http_0_2_0_types_method_outgoing_request_set_authority(borrow,
                                                                              maybe_authority);

    // TODO: error handling on result
    wasi_http_0_2_0_types_method_outgoing_request_set_path_with_query(
...
Result<FutureHttpIncomingResponse *> HttpOutgoingRequest::send() {
  ...
  wasi_http_0_2_0_outgoing_handler_handle({handle_state_->handle}, nullptr, &ret,
                                                        &err);
...
Result<HttpIncomingBody::ReadResult> HttpIncomingBody::read(uint32_t chunk_size) {
  ...
  bool success =
      wasi_io_0_2_0_streams_method_input_stream_read(borrow, chunk_size, &ret, &err);
  if (!success) {
...
Result<optional<HttpIncomingResponse *>> FutureHttpIncomingResponse::maybe_response() {
  ...
  auto borrow =
      wasi_http_0_2_0_types_borrow_future_incoming_response({handle_state_->handle});
...
Result<PollableHandle> FutureHttpIncomingResponse::subscribe() {
  auto borrow =
      wasi_http_0_2_0_types_borrow_future_incoming_response({handle_state_->handle});
  auto pollable =
      wasi_http_0_2_0_types_method_future_incoming_response_subscribe(borrow);
  return Result<PollableHandle>::ok(pollable.__handle);
}
...
Result<uint16_t> HttpIncomingResponse::status() {
  if (status_ == UNSET_STATUS) {
    ...
    auto borrow =
        wasi_http_0_2_0_types_borrow_incoming_response_t({handle_state_->handle});
...
Result<HttpOutgoingBody *> HttpOutgoingResponse::body() {
  ...
  if (!this->body_) {
    ...
    if (!wasi_http_0_2_0_types_method_outgoing_response_body(
            wasi_http_0_2_0_types_borrow_outgoing_response({handle_state_->handle}),
            &body)) {
      return Res::err(154);
    }
    this->body_ = new HttpOutgoingBody(body.__handle);
