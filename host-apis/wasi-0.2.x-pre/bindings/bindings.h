...
typedef wasi_io_error_own_error_t wasi_http_types_own_io_error_t;

typedef struct {
  bool is_err;
  union {
    wasi_http_types_own_io_error_t err;
  } val;
} wasi_http_types_result_void_own_io_error_t;
...
// Append an `input-stream` to an `outgoing-body`, without blocking.
// 
// This is related to `output-stream.splice`, but instead of reading a
// range of bytes from the `input-stream` it takes ownership and
// asynchronously appends the entire `input-stream` to the `outgoing-body`.
// 
// Can be called multiple times without waiting for previous `append`
// operations to complete. In this case, `append` operations are queued
// up and start appending at the position in the `outgoing-body` that's
// current when the operation starts.
// 
// Calling `write` will fail after calling `append`, so it's not possible
// to mix writes to the underlying `output-stream` with `append` operations.
// 
// If a `len` value is provided, the implementation should read exactly that
// number of bytes from the `input-stream`. If the `input-stream` is shorter,
// the implementation should treat the body as corrupted and abort sending it,
// even if additional `append` operations are pending.
extern bool wasi_http_types_method_outgoing_body_append(wasi_http_types_borrow_outgoing_body_t self, wasi_http_types_own_input_stream_t src, uint64_t *maybe_len, wasi_http_types_own_io_error_t *err);
...
// to the body (via `write` or the `append` operation) does not match the
// value given in the Content-Length. In case of the `append` operation,
// the sum of the `len` values passed to `append` must match the Content-Length.
