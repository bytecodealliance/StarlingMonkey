...
Result<HttpHeaders *> HttpHeaders::FromEntries(vector<tuple<HostString, HostString>> &entries) {
  std::vector<bindings_tuple2_field_name_field_value_t> pairs;
  ...
  bindings_list_tuple2_field_name_field_value_t tuples{pairs.data(), entries.size()};
...
Result<vector<tuple<HostString, HostString>>> HttpHeadersReadOnly::entries() const {
  ...
  bindings_list_tuple2_field_name_field_value_t entries;
